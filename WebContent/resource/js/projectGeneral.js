...
function saveCheck02(callback){
	if(isEdited){
		if(confirm("저장하지 않은 항목이 있습니다. 저장하지 않은 데이터는 유실됩니다. \n" +
				"계속 진행하시겠습니까?")){
			//isEdited = false;
...
function saveCheck04(callback){
	if(isEdited){
		$(".pop_movePg04Confirm").dialog("open");
		if(confirm("저장하지 않은 항목이 있습니다. 저장하지 않은 데이터는 유실됩니다. \n" +
				"계속 진행하시겠습니까?")){
			isEdited = false;
	 ...
	 $.ajax({
			...
			success: function(data) {
				if(data[0]==='파일 존재'){
					...
					$('#tr_cover_uploadFileInfo').html('<a style="text-decoration:underline;color:blue;" href="'+current_server_domain+'/downloadTRCoverTemplateFile.do?prj_id='+encodeURIComponent(getPrjInfo().id)+'&rfile_nm='+encodeURIComponent(rfile_nm)+'">'+rfile_nm+'</a>');
				}else{
					$('#tr_cover_uploadFileInfo').html('');
				}
				if(data[1]==='파일 존재'){
					let rfile_nm = getPrjInfo().nm+'_p_vtr.xls';
					$('#vtr_cover_uploadFileInfo').html('<a style="text-decoration:underline;color:blue;" href="'+current_server_domain+'/downloadTRCoverTemplateFile.do?prj_id='+encodeURIComponent(getPrjInfo().id)+'&rfile_nm='+encodeURIComponent(rfile_nm)+'">'+rfile_nm+'</a>');
				}else{
					$('#vtr_cover_uploadFileInfo').html('');
				}
				if(data[2]==='파일 존재'){
					let rfile_nm = getPrjInfo().nm+'_p_str.xls';
					$('#str_cover_uploadFileInfo').html('<a style="text-decoration:underline;color:blue;" href="'+current_server_domain+'/downloadTRCoverTemplateFile.do?prj_id='+encodeURIComponent(getPrjInfo().id)+'&rfile_nm='+encodeURIComponent(rfile_nm)+'">'+rfile_nm+'</a>');
				}else{
...
$(function(){
	...
	$(document).on("click", "#projectGeneral02ALink", function () {
		$.ajax({ // p_prj_sys_log에 정보 저장
		    ...
		    data: {
		    	prj_id: $('#selectPrjId').val(),
		    	pgm_nm: "PROJECT GENERAL > 프로세스 TYPE 정의 폴더 MATCH DRN 사용여부 설정",
		    	method: "조회"
		    },
		    success: function onData () {
	...
	$(document).on("click", "#projectGeneral03ALink", function () {
		$.ajax({ // p_prj_sys_log에 정보 저장
		    ...
		    data: {
		    	prj_id: $('#selectPrjId').val(),
		    	pgm_nm: "PROJECT GENERAL > NUMBERING 설정",
		    	method: "조회"
		    },
		    success: function onData () {
	...
	$(document).on("click", "#projectGeneral04ALink", function () {
		$.ajax({ // p_prj_sys_log에 정보 저장
		    ...
		    data: {
		    	prj_id: $('#selectPrjId').val(),
		    	pgm_nm: "PROJECT GENERAL > DOCUMENT, TRANSMITAL ISSCUE, RETURN 필드값 정의",
		    	method: "조회"
		    },
		    success: function onData () {
	...
	$(document).on("click", "#projectGeneral07ALink", function () {
		$.ajax({ // p_prj_sys_log에 정보 저장
		    ...
		    data: {
		    	prj_id: $('#selectPrjId').val(),
		    	pgm_nm: "PROJECT GENERAL > DISCIPLINE 설정, 폴더 MATCH",
		    	method: "조회"
		    },
		    success: function onData () {
	...
	$(document).on("click", "#projectGeneral08ALink", function () {
		$.ajax({ // p_prj_sys_log에 정보 저장
		    ...
		    data: {
		    	prj_id: $('#selectPrjId').val(),
		    	pgm_nm: "PROJECT GENERAL > PROJECT USER/GROUP 설정",
		    	method: "조회"
		    },
		    success: function onData () {
		...
		window.open('./pop/projectGeneral08.jsp','PROJECT USER/GROUP 설정','top=50, left=50, width=1700, height=850, resizable=yes');
	});
	$(document).on("click", "#projectGeneral09ALink", function () {
		...
		$.ajax({ // p_prj_sys_log에 정보 저장
		    ...
		    data: {
		    	prj_id: $('#selectPrjId').val(),
		    	pgm_nm: "PROJECT GENERAL > 폴더별 권한 설정",
		    	method: "조회"
		    },
		    success: function onData () {
	...
	$(document).on("click", "#projectGeneral11ALink", function () {
		$.ajax({ // p_prj_sys_log에 정보 저장
		    ...
		    data: {
		    	prj_id: $('#selectPrjId').val(),
		    	pgm_nm: "PROJECT GENERAL > Transmittal Cover Template 설정",
		    	method: "조회"
		    },
		    success: function onData () {
	...
	*/
    ...
    fileTarget.on('change', function () {
        ...
        let fileNameArr = $(this)[0].files[0].name.split("\.");
        let ext = fileNameArr[fileNameArr.length - 1];
        
        //console.log("ext = "+ext);
    	if(ext != 'xls'){
            // 확장자 체크
            alert(".xls 확장자 파일만 업로드 할 수 있습니다.");
        }else {
...
function folderRefresh(){
	...
	var data1 = getFolderListForPg01();
	data1 = dccFolder(data1);
	tree.settings.core.data = data1;
	tree.refresh();
...
var newFlag = false;
function getPGTree(){
	var folderList = getFolderListForPg01();
	...
	function jstreeContextMenu(node) {
	    ...
	    if(adminAndDCCArray.includes(session_user_id)){	//Admin이나 DCC일 때만 트리메뉴 수정가능
		    if (node.parent=='#' && node.original.type=='DCC') {
		    	...
		    	//items.pasteItem._disabled = true; //ROOT폴더도 paste기능 필요하다해서 주석처리함
	...
	var pg01_tree = $('#pg01_tree').jstree({
	     "core" : {
	    	 'data' : folderList,
	         "check_callback" : true
	       },
...
function pgen_step02(){	
	...
	$("#pg02_btnPath").click(function(){
		...
		let getSelectedItem = pg02.grid.getSelectedData()
		let folder_path_id = null;
		if(getSelectedItem){
			folder_path_id = Number(getSelectedItem[0].process_folder_path_id);	
		}
		...
		$('#pg02_tree').jstree("deselect_all");
		if(folder_path_id != null)
			$('#pg02_tree').jstree("select_node",folder_path_id);
		tree.refresh();
	});
...
//	console.log(rowIndex);
	var selected = $('#pg02_tree').jstree('get_selected', true); 
		//pg02_Tree.jstree('get_selected', true);
...
function pg02_getGrid(data){
	...
	pg02.grid.on("rowClick", function(e, row){
		...
		if(pg02_selectdData.class_lvl == "SYSTEM")
			$("#pg02_setDRN")[0].disabled = true;
		else if(pg02_selectdData.process_type.includes('LETTER'))
			$("#pg02_setDRN")[0].disabled = true;
		else if(pg02_selectdData.process_type.includes('E-MAIL'))
			$("#pg02_setDRN")[0].disabled = true;
		else if(pg02_selectdData.process_type.includes('PROCUREMENT (POC)'))
			$("#pg02_setDRN")[0].disabled = true;
		else if(pg02_selectdData.process_type.includes('BULK MATERIAL (BMC)'))
			$("#pg02_setDRN")[0].disabled = true;
		else
			$("#pg02_setDRN")[0].disabled = false;
		
		if(pg02_selectdData.bean){
		...
		$("#pg02_setPType").change(function(){
			...
			let p_type = $("#pg02_setPType").val();
			
			if(p_type.includes("LETTER")){
				$("#pg02_setDRN").val('N');
				$("#pg02_setDRN")[0].disabled = true;
			}else if(p_type.includes("E-MAIL")){
				$("#pg02_setDRN").val('N');
				$("#pg02_setDRN")[0].disabled = true;
			}else if(p_type.includes("PROCUREMENT (POC)")){
				$("#pg02_setDRN").val('N');
				$("#pg02_setDRN")[0].disabled = true;
			}else if(p_type.includes("BULK MATERIAL (BMC)")){
				$("#pg02_setDRN").val('N');
				$("#pg02_setDRN")[0].disabled = true;
			}else{
				$("#pg02_setDRN")[0].disabled = false;
				if(pg02_selectdData){
					let drn_use_yn = pg02_selectdData.bean ? pg02_selectdData.bean.drn_use_yn : "N";
					$("#pg02_setDRN").val(drn_use_yn);
				}
			}
...
function pg02_apply(prjInfo){
	...
	if(!pj_no || !pj_nm || !pj_full_nm) {
		...
		return;
	}else if(pj_no.length > 10) {
		alert("PROJECT CODE는 최대 10자까지 가능합니다.");
		return;
	}
...
function pg02_add2(prjInfo){
	...
	if(addChk > pg02.size) {
		pg02.grid.selectRow(pg02.grid.getRowFromPosition(pg02.size));
		return;
	}
    ...
    pg02.grid.selectRow(pg02.grid.getRowFromPosition(addChk));
...
function pg02_save(prjInfo){
	...
	if(!process_type){
		alert("the process type must be selected");
    ...
    // DRN 적용하지 않는 프로세스 타입
    if(selectedData){
    	let p_type = selectedData.process_type;
    	switch(p_type){
    		case 'LETTER':
    			drn_use_yn = 'N';
    			break;
    		case 'E-MAIL':
    			drn_use_yn = 'N';
    			break;
    		case 'PROCUREMENT (POC)':
    			drn_use_yn = 'N';
    			break;
    		case 'BULK MATERIAL (BMC)':
    			drn_use_yn = 'N';
    			break;
    		default:
    	}
    	...
    	/*
    	if(p_type.includes('LETTER')) drn_use_yn = 'N';
    	else if(p_type.includes('E-MAIL')) drn_use_yn = 'N';
    	else if(p_type.includes('PROCUREMENT (POC)')) drn_use_yn = 'N';
    	else if(p_type.includes('BULK MATERIAL (BMC)')) drn_use_yn = 'N';
    	*/
    }
...
//	}
	...
	let checkedRows = $('input:checkbox[name="pg02_gridChk"]:checked');
	if(checkedRows.length<1){
		alert("Please check the row to delete");
		return;
	}
	
	$(".pop_deletePg02Confirm").dialog("open");
//	$('#deleteCode02Yes').click(function() {
//		$('.pop_deletePg02Confirm').dialog("close");
//		
//		$.ajax({ // p_prj_sys_log에 정보 저장
//			url: 'insertSystemLog.do',
//		    type: 'POST',
//		    data: {
//		    	prj_id: $('#selectPrjId').val(),
//		    	pgm_nm: "PROJECT GENERAL > 프로세스 TYPE 정의 폴더 MATCH DRN 사용여부",
//		    	method: "삭제"
//		    },
//		    success: function onData () {
//		    	
...
//		});
//	});
	$('#deleteCode02Cancel').click(function() {
    	$(".pop_deletePg02Confirm").dialog("close");
    });
	
}

function deleteCode02Yes() {
	// 체크된 row 확인
	var checkedRows = $('input:checkbox[name="pg02_gridChk"]:checked');
	...
	for(var i=0;i<checkedRows.length;i++){
		...
		$.ajax({
		    ...
		    type: 'POST',
		    data:{
		    	prj_id:setPg02PrjInfo,
		    	process_seq_list:process_seq_list
		    },
		    success: function onData (data) {
		    	if(data[2]>0){
		    		alert("Delete complete except can't delete for there are documents in that folder or subfolder of process type");
		    	}
		    	else{
		    		alert("There are documents in this folder or subfolder of this process type. So can't delete");
		    	}
...
function pgen_step03(){
    ...
    initPrjSelectGridPg03Copy();
	
}

function pg03_serial_apply(){
	let set_val4 = $("#pg03_SERIAL").val().trim();
...
function initPrjSelectGridPg03Copy() {
	prj_grid_pg03_copy  = new Tabulator("#prj_list_grid_pg03", {
		layout: "fitColumns",
	...
	prj_grid_pg03_copy.on("tableBuilt", function(){
		$.ajax({
		    url: 'getPrjGridList.do',
		    type: 'POST',
		    success: function onData (data) {
		    	prj_grid_pg03_copy.setData(data.model.prjList);
		    },
		    error: function onError (error) {
		        console.error(error);
		    }
	...
	prj_grid_pg03_copy.on("rowClick",function(e,row){
		let rowData = row.getData();
		$("#pg03_selectPrjId").val(rowData.prj_id);
		$("#pg03_selected_prj").html(rowData.prj_nm);
	});
	...
	$(document).keydown(function(event) {
		...
		if(document.getElementById('prjGen_search') == focusEle){
 ...
 * @returns
 */
function prjGenSearch() {
	let keyword = $("#prjGen_search").val();
	let grid = prj_grid_pg03_copy;
...
var valueEditor = function(cell, onRendered, success, cancel){
    ...
    function onChange(){
        if(input.value != cellValue){
        	/*if(input.value.indexOf("%")!==-1 || input.value.indexOf("/")!==-1 || input.value.indexOf("\\")!==-1){
        		input.value = input.value.replaceAll('%','').replaceAll('/','').replaceAll('\\','');        		
        		$('.numberingCheck').html($('.numberingCheck').html()+'"%"와 "/"와 "\\"는 입력할 수 없습니다.');
        	}*/
        	if(input.value.indexOf("\\")!==-1){
        		input.value = input.value.replaceAll('\\','');        		
        		$('.numberingCheck').html($('.numberingCheck').html()+'"\\"는 입력할 수 없습니다.');
        	}
            success(input.value);	
        }else{
            cancel();
	...
	for(let i=0;i<$(".pg03_Grids").length;i++){
		...
		pg03.dial_Grid[i].on("tableBuilt", function(){
			...
			case 2:
				...
				pg03.dial_Grid[i].getColumn('set_code').updateDefinition({visible:true,title:'PROCESS TYPE',editor:'select'});
				pg03.dial_Grid[i].getColumn('set_val').updateDefinition({visible:true,title:'OUTGOING NUMBERING',editor:valueEditor});
				break;
			case 3:
				pg03.dial_Grid[i].getColumn('set_valS').updateDefinition({visible:false});
				pg03.dial_Grid[i].getColumn('set_desc').updateDefinition({visible:true,title:"DESCRIPTION",editor:'input'});
				pg03.dial_Grid[i].getColumn('set_code').updateDefinition({visible:true,title:'PROCESS TYPE',editor:'select'});
				pg03.dial_Grid[i].getColumn('set_val').updateDefinition({visible:true,title:'INCOMING NUMBERING',editor:valueEditor});
				break;
			default:
				break;
...
function pg03_save(gridIdx){
	...
	if(gridIdx>1){
		...
		for(let i=0 ; i<chkData.length ; i++){
			if(!/\d/.test(chkData[i].set_val)){ //numbering체계에 숫자가 들어있지 않으면 return
				alert('넘버링 체계에는 숫자가 반드시 포함되어야 합니다.');
				return;
			}
		}
	}
...
function pgen_step04(){
	...
	$(pg04.btnList[1]).click(function(){
		if(isEdited == true) {
			...
			$(".pop_RefershPg04Confirm").dialog("open");
	...
	*/
    ...
    initPrjSelectGridPg04Copy();
}

function refreshCodeYes() {
	$(".pop_RefershPg04Confirm").dialog("close");
	isEdited = false;
	let i = $("#pg04_codeType").find(":selected").index();
	getPg04Grid(i);
}

function refreshCodeCancel() {
	$(".pop_RefershPg04Confirm").dialog("close");
}

function initPrjSelectGridPg04Copy() {
	prj_grid_pg04_copy  = new Tabulator("#prj_list_grid_pg04", {
		layout: "fitColumns",
		placeholder:"There is No Project",
		selectable:1,
		index:"prj_id",
	...
	prj_grid_pg04_copy.on("tableBuilt", function(){
		$.ajax({
		    url: 'getPrjGridList.do',
		    type: 'POST',
		    success: function onData (data) {
		    	prj_grid_pg04_copy.setData(data.model.prjList);
		    },
		    error: function onError (error) {
		        console.error(error);
		    }
	...
	prj_grid_pg04_copy.on("rowClick",function(e,row){
		let rowData = row.getData();
		$("#PG04_selectPrjId").val(rowData.prj_id);
		$("#PG04_selected_prj").html(rowData.prj_nm);
	});
	...
	$(document).keydown(function(event) {
		 // 프로젝트 검색 input영역에 포커스가 가있는 경우에만 작동
		let focusEle = document.activeElement;
		if(document.getElementById('prjGen04_search') == focusEle){
			 if ( event.keyCode == 13 || event.which == 13 ) {
				 	prjGen04Search();
			    }
		}
 ...
 * @returns
 */
function prjGen04Search() {
	let keyword = $("#prjGen04_search").val();
	let grid = prj_grid_pg04_copy;
...
//	}
	...
	var tmpCol = [
	    ...
	    {
	        title: "PROGRESS STATUS 제외",
	        field: "set_val2",
			formatter: function(cell, formatterParams, onRendered) {
				
		        let cellValue = cell.getValue();	
		         var checkbox = document.createElement("input");
		          checkbox.id = 'pg04_progress_status_use_yn'+cell.getRow().getPosition();
		          checkbox.className = 'pg04_progress_status_use_yn';
		          checkbox.type = 'checkbox';
		          checkbox.checked = (cellValue == 'N');
		          console.log(cellValue);
		          console.log( (cellValue == 'N') );
	...
	$.ajax({
	    ...
	    success: function onData (data) {
	    	...
	    	for(var i=0;i<data[0].length;i++){
	    		...
	    		tmp.set_val2 = data[0][i].set_val2;
	    		inputData.push(tmp);
	    	}
...
//	    	console.log("선택한 select = "+gridIndex);
	    	...
	    	if(gridIndex == 0) { // 리비전 넘버
	    		...
	    		$("#sel_VTRReturnStep").css('display','none');
	    		$("#sel_STRIssue").css('display','none');
	    		$("#sel_STRReturn").css('display','none');
	    		$("#sel_RevisionNumber").css('display','flex');
	    	}else if(gridIndex == 1) { //doc_size
	    	...
	    	}else if(gridIndex == 2) { // vendor doc size
	    		$("#sel_DocSize").css('display','none');
	    		$("#sel_DocCat").css('display','none');
	    		$("#sel_DocType").css('display','none');
	    		...
	    		$("#sel_STRReturn").css('display','none');
	    		$("#sel_RevisionNumber").css('display','none');
	    	}else if(gridIndex == 3) { // tr issue purpose
	    		$("#sel_DocSize").css('display','none');
	    		$("#sel_DocCat").css('display','none');
	    		$("#sel_DocType").css('display','none');
	    		$("#sel_VDocSize").css('display','none');
	    		...
	    		$("#sel_STRReturn").css('display','none');
	    		$("#sel_RevisionNumber").css('display','none');
	    	}else if(gridIndex == 4) { // vtr issue purpose
	    		$("#sel_DocSize").css('display','none');
	    		$("#sel_DocCat").css('display','none');
	    		$("#sel_DocType").css('display','none');
	    		$("#sel_VDocSize").css('display','none');
	    		...
	    		$("#sel_STRReturn").css('display','none');
	    		$("#sel_RevisionNumber").css('display','none');
	    	}else if(gridIndex == 5) { //str issue purpose
	    		$("#sel_DocSize").css('display','none');
	    		$("#sel_DocCat").css('display','none');
	    		$("#sel_DocType").css('display','none');
	    		$("#sel_VDocSize").css('display','none');
	    		...
	    		$("#sel_STRReturn").css('display','none');
	    		$("#sel_RevisionNumber").css('display','none');
	    	}else if(gridIndex == 7) { //vendor doc cate
	    		$("#sel_DocSize").css('display','none');
	    		$("#sel_DocCat").css('display','none');
	    		$("#sel_DocType").css('display','none');
	    		$("#sel_VDocSize").css('display','none');
	    		...
	    		$("#sel_STRReturn").css('display','none');
	    		$("#sel_RevisionNumber").css('display','none');
	    	}else if(gridIndex == 8) { // tr return status
	    		$("#sel_DocSize").css('display','none');
	    		$("#sel_DocCat").css('display','none');
	    		$("#sel_DocType").css('display','none');
	    		$("#sel_VDocSize").css('display','none');
	    		...
	    		$("#sel_STRReturn").css('display','none');
	    		$("#sel_RevisionNumber").css('display','none');
	    	}else if(gridIndex == 9) { // vtr return status
	    		$("#sel_DocSize").css('display','none');
	    		$("#sel_DocCat").css('display','none');
	    		$("#sel_DocType").css('display','none');
	    		$("#sel_VDocSize").css('display','none');
	    		...
	    		$("#sel_STRReturn").css('display','none');
	    		$("#sel_RevisionNumber").css('display','none');
	    	}else if(gridIndex == 10) { // str return status
	    		$("#sel_DocSize").css('display','none');
	    		$("#sel_DocCat").css('display','none');
	    		$("#sel_DocType").css('display','none');
	    		$("#sel_VDocSize").css('display','none');
	    		...
	    		$("#sel_VTRReturnStep").css('display','none');
	    		$("#sel_STRIssue").css('display','none');
	    		$("#sel_STRReturn").css('display','flex');
	    		$("#sel_RevisionNumber").css('display','none');	    		
	    	}else if(gridIndex == 11) { // doc type
	    		$("#sel_DocSize").css('display','none');
	    		$("#sel_DocCat").css('display','none');
...
//	console.log(set_code_type);
	...
	}else if(set_code_type == "VTR RETURN STATUS") {
		fileName = 'VTR_Return_Status_Code';
	}else if(set_code_type == "STR RETURN STATUS") {
		fileName = 'STR_Return_Status_Code';
...
function pg04_Apply(){
	...
	if(gridIndex==6 || gridIndex==7){
		let categoryLength = $('.pg04_progress_status_use_yn').length;
		for(let i=0;i<categoryLength;i++){
			if($('#pg04_progress_status_use_yn'+i).is(":checked")){
				tmpGridData[i].bean.set_val2 = 'N';
				tmpStrings.push( JSON.stringify(tmpGridData[i].bean) );
			}else{
				tmpGridData[i].bean.set_val2 = 'Y';
				tmpStrings.push( JSON.stringify(tmpGridData[i].bean) );
			}
		}
	}
...
function pg04_Del(){ // 아래 삭제시 onclick 방식으로 변경
	$('.pop_deletePg04Confirm').dialog("open");
    $('#deleteCode04Cancel').click(function() {
    	$(".pop_deletePg04Confirm").dialog("close");
    });
}

function deleteCodeYes() {
	var gridIndex = $("#pg04_codeType").find(":selected").index();
	var thisGrid = pg04.grid[gridIndex];
	...
	// 바로삭제 적용
	 $.ajax({
			...
			success: function(data) {
				...
				// 사용중인 코드라서,삭제하지 못한 값 제외
				for(let i=0;i<delval.length;i++)
					delRowsData = delRowsData.filter(item => item.bean.set_code_id != delval[i]);
				
				// 실질적으로 삭제된 코드들만 row에서 제거
				for(let i=0;i<delRowsData.length;i++)
					thisGrid.deleteRow(delRowsData[i].index);
				...
				// alert 문구 설정
				if(delval.length>0)
					alert("delete complete except code being used");
				else
					alert("delete complete.");
			},
		    error: function onError (error) {
...
var pg07 = { isFirst : true, isAddMode : false};
var prj_grid_pg07_copy;
...
function pgen_step07(){
	...
	initPrjSelectGridPg07Copy();
}

function initPrjSelectGridPg07Copy() {
	prj_grid_pg07_copy  = new Tabulator("#prj_list_grid_pg07", {
		layout: "fitColumns",
		placeholder:"There is No Project",
		selectable:1,
		index:"prj_id",
	...
	prj_grid_pg07_copy.on("tableBuilt", function(){
		$.ajax({
		    url: 'getPrjGridList.do',
		    type: 'POST',
		    success: function onData (data) {
		    	prj_grid_pg07_copy.setData(data.model.prjList);
		    },
		    error: function onError (error) {
		        console.error(error);
		    }
	...
	prj_grid_pg07_copy.on("rowClick",function(e,row){
		let rowData = row.getData();
		$("#pg07_selectPrjId").val(rowData.prj_id);
		$("#pg07_selected_prj").html(rowData.prj_nm);
	});
	...
	$(document).keydown(function(event) {
		 // 프로젝트 검색 input영역에 포커스가 가있는 경우에만 작동
		let focusEle = document.activeElement;
		if(document.getElementById('prjGen07_search') == focusEle){
			 if ( event.keyCode == 13 || event.which == 13 ) {
				 	prjGen07Search();
			    }
		}
 ...
 * @returns
 */
function prjGen07Search() {
	let keyword = $("#prjGen07_search").val();
	let grid = prj_grid_pg07_copy;
...
function pg07_Add(){
	...
	$.ajax({
		    ...
		    success: function onData (data) {
		    	...
		    	pg07.grid[0].addRow(pg07.selectedData);
		    	pg07.grid[0].redraw();
		    	
		    	let lastIdx = pg07.grid[0].getData().length - 1;
		    	let row = pg07.grid[0].getRowFromPosition(lastIdx)
		    	pg07.grid[0].selectRow(row);
		    	pg07.isAddMode = true;
		    	pg07.addRowIdx = lastIdx;
		    },
		    error: function onError (error) {
		        console.error(error);
		    }
...
function pg07_Save(){
	...
	if(pg07.selectedData.discip_code == ''){
		alert("Please input discipline code");
		return;
	}
	if(pg07.selectedData.discip_code == ''){
		alert("Please input discipline display");
		return;
	}
	if(pg07.selectedData.discip_code == ''){
		alert("Please input discipline description");
		return;
	}
	
	if(pg07.selectedData.discip_folder_id_List.length == 0){
		alert("Please select the folder");
		return;
	}else if(pg07.selectedData.discip_folder_id_List[0] == null){
		alert("Please select the folder");
		return;
	}
...
//	console.log(pg07.selectedData);
	...
	// Add mode Off
	let selectedRowIdx = pg07.grid[0].getSelectedRows()[0].getPosition();
	if(selectedRowIdx == pg07.addRowIdx){
		pg07.isAddMode = false;
		pg07.addRowIdx = "";
	}
...
function pg07_Del(){
	...
	$(".pop_deletePg07Confirm").dialog("open");
	
	$('#deleteCode07Cancel').click(function() {
    	$(".pop_deletePg07Confirm").dialog("close");
    });	
}

function deleteCode07Yes() {
	$('.pop_deletePg07Confirm').dialog("close");
	$.ajax({ // p_prj_sys_log에 정보 저장
		url: 'insertSystemLog.do',
	    type: 'POST',
