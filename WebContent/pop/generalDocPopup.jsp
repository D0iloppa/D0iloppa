                        ...
                        <div class="pg_top">
                             <div class="left">
								...
								<button class="button btn_orange" onclick="getGeneralAccessHisList()">
                                    ...
                                    <button class="button btn_orange" id="general_delete" onclick="docDelete('General')">
                                        Delete
                                    </button>
							</div>
                            <!-- <div class="right">
                            ...
                            <table class="write">
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <div class="multi_sel_wrap" id="generalRevNoSelectWrap">
                                ...
                                <tr>
                                    ...
                                    <td colspan="3">
										...
										<form id="updateRevisionGeneralForm" style="display:flex;" name="updateRevisionGeneralForm"  enctype="multipart/form-data" method="post" onsubmit="return false;">
					                    	<span id="rfile_download" class="property_attach" onclick="propertydownload()"></span>
					                        <span class="filebox">
					                        	<input type="text" id="general_uploadFileDate" disabled style="width: 190px;">
...
var trans_ref_doc_id = opener.document.getElementById('trans_ref_doc_id').value;
...
$(function(){
	...
	if(auth.w==='Y'){
		$('#general_revise').css('display','inline-block');
		$('#general_save').css('display','inline-block');
		$('#generalNewButton').css('display','inline-block');
	}else{
		$('#general_revise').css('display','none');
		$('#general_save').css('display','none');
		$('#generalNewButton').css('display','none');
	}
	if(auth.d==='Y'){
		$('#general_delete').css('display','inline-block');
	}else{
		$('#general_delete').css('display','none');
	}
    ...
    fileTarget.on('change', function () {
        ...
        var today = new Date();
        $('#general_uploadFileDate').val(formatBytes(size,2)+', '+today.toLocaleString());
	...
	if(process==='new'){
		$('#designerSelectConfirm').attr('onclick','designerSelectConfirm(`General`)');
        $(".pop_generalDocNew").dialog("open");
    	$('#popup_general_current_folder_path').html(current_folder_path);
    	$('#generalRevNoSelected').css('pointer-events', 'none');
    	$('#generalRevNoSelected').css('background', 'none');
    	$('#generalRevNoSelected').css('background-color', '#FAFAFA');
    	$('#generalRevNoSelected').css('color', '#FAFAFA');
    	getRevNoMultiBox('General');
    	...
    	$('#generalRevNoSelected').css('color', '#000');
...
function setPopupPrjDocumentIndexList(doc_ctrl_li,type){
	...
	thisTable.on("rowSelected", function(row){
		...
		if(type==='DCI'){
			...
			getGeneralDocumentInfo(eng_selected_doc_id,eng_selected_rev_id);
			...
			if(selected_doc_type==='General'){
				...
				alert('해당 문서는 General 타입 문서가 아닙니다.');
			}
		}
	});
}
	...
	$.ajax({
	    ...
	    success: function onData (data) {
	    	...
	    	if(data[0].rfile_nm!==null && data[0].rfile_nm!==undefined && data[0].rfile_nm!==''){
		    	...
		    	$('#general_uploadFileDate').val(formatBytes(data[0].file_size,2)+attach_file_date);
	    	...
	    	$('#general_creation').val(data[0].reg_nm+'('+data[0].reg_kor_nm+'): '+getDateFormat(data[0].reg_date));
	    	$('#general_modification').val(data[0].mod_date===null?'':data[0].mod_nm+'('+data[0].mod_kor_nm+'): '+getDateFormat(data[0].mod_date));
	    },
	    error: function onError (error) {
	        console.error(error);
	    }
...
function docDeleteConfirm(){
	...
	let doc_title = $('#general_title').val();
...
function popupRefreshButton(type){
	...
	if(DCCorTRA==='DCC'){
		...
		if(type==='DCI'){
			...
			getGeneralDocumentInfo(doc_id,rev_id);	//General의 경우 trans_ref_doc_id를 보면 안됨
		}
	}
...
function setGeneralNull(){
	...
	$('#general_title').val('');
	$('#general_creation').val(session_user_eng_nm+'('+session_user_kor_nm+'): '+ $('#currentTime').val());
	$('#general_modification').val(session_user_eng_nm+'('+session_user_kor_nm+'): '+ $('#currentTime').val());
	$('#general_uploadFileDate').val('');
}
...
function generalDocSave(){
	...
	if(doc_no==='' || doc_no===null || doc_no===undefined){
	    ...
	    formData.set('trans_ref_doc_id',trans_ref_doc_id);
	    if(file_value!==''){
		 ...
		 $.ajax({
				...
				success: function(data) {
					if(data[0]==='확장자 오류'){
						...
						if(trans_ref_doc_id!=='COVER' && data[0]>0){
							getPopupPrjDocumentIndexList('General');
							getGeneralDocumentInfo(data[1],data[2]);
							general_selected_doc_id = data[1];
							general_selected_rev_id = data[2];
						...
						}else if(trans_ref_doc_id==='COVER'){	//COVER 파일은 gendoc_info테이블에 데이터가 없으므로 업데이트 할 수 없음. 따라서 data[0]=0이므로 체크하지 않음
							getPopupPrjDocumentIndexList('General');
							getGeneralDocumentInfo(data[1],data[2]);
							general_selected_doc_id = data[1];
							general_selected_rev_id = data[2];
				...
				success: function(data) {
					if(data[0]==='확장자 오류'){
						...
						if(data[0]>0){
							...
							$('#general_save').attr('disabled',false);
							$('#general_revise').attr('disabled',true);
							getGeneralDocumentInfo(doc_id,data[1]);
							getRevNoMultiBox('General',doc_id);
							alert('CheckIn Completed');
							general_selected_rev_id = data[1];
...
function propertydownload(){
	$.ajax({
	    ...
	    data:{
	    	prj_id:selectPrjId,
	    	folder_id:current_folder_id,
	    	doc_id:general_selected_doc_id,
	    	rev_id:general_selected_rev_id,
	    	inout:''
	    },
	    success: function onData (data) {
			let multiDownTbl = [];
